# Claude Integration Prompt — the learner Standards Library v2

Inspect the existing BRIDGE Learning Agent before changing code.

Add `grade4_standards_library_v2.json` as a REFERENCE LAYER, not a curriculum generator.
...
Hard rules:
1. The school task decides what the learner learns.
...
6. Do not assign the learner a fixed WIDA proficiency level from one task.
...
9. Use the minimum scaffold that preserves the learner's thinking.
...
H. how WIDA evidence is stored without labeling the learner globally.
